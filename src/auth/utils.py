...
def mft_GenerateToken(email: str, expires_sec: int = 3600) -> str:
    ...
    s = URLSafeTimedSerializer(current_app.mft_Config["SECRET_KEY"])
    return s.dumps(email, salt=current_app.mft_Config.get("SECURITY_SALT"))
...
def mft_VerifyGenerateToken(token: str, max_age: int = 3600) -> str | None:
    ...
    s = URLSafeTimedSerializer(current_app.mft_Config["SECRET_KEY"])
    try:
        email = s.loads(token, salt=current_app.mft_Config.get("SECURITY_SALT"), max_age=max_age)
        return email
    except Exception:
        return None
